...
//    }
    ...
    public void arvore(String caminhoAtual, String barra) throws IOException {
        barra = barra + barra;
        String directory = caminhoAtual;
        try (DirectoryStream<Path> directoryStream = Files.newDirectoryStream(
                Paths.get(directory))) {
...
//                teste(file);
                if (file.isDirectory()) {
                    System.out.println((barra) + "->" + file.getName());
                    arvore(file.getPath(),barra);
                } else {
                    System.out.println((barra + barra + ":") + file.getName());
                }
            }
        }
    }

    private void teste(File file) {

        if (file.isDirectory()) {
            System.out.println("->" + file.getName());
            teste(file.getParentFile());
        } else {
            System.out.println("---:" + file.getName());
        }
    }
